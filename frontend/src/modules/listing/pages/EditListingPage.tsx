...
import type { UpdateListingRequest, ListingDetailResponse, TagDto } from '../../../types/listing';
...
export default function EditListingPage() {
  ...
  useEffect(() => {
    ...
    listingService.getById(Number(id))
      .then((data: ListingDetailResponse) => {
        setForm({
          title: data.title,
          ...
          rentPrice: data.priceRent,
          electricPrice: data.priceElectricity ? data.priceElectricity : '',
          waterPrice: data.priceWater ? data.priceWater : '',
          deposit: '', // Deposit not in detail response
          internetPrice: '', // Not in detail response
          otherFees: '', // Not in detail response
          amenities: new Set(data.facilities.map(f => f.name as AmenityKey)),
          tags: new Set(data.tags?.map(t => t.slug) || []),
          imageUrls: data.images.sort((a, b) => a.sortOrder - b.sortOrder).map(img => img.url),
          maxOccupants: data.maxOccupants ? data.maxOccupants : '',
          floorNumber: '',
          totalFloors: '',
        });
      })
      .catch(err => {
        ...
        setSubmitError('Không thể tải thông tin phòng.');
      })
  ...
  const handleNext = async () => {
    ...
    if (step < STEPS.length - 1) {
      ...
      try {
        await listingService.update(Number(id), request);
        setSubmitted(true);
      } catch (err: unknown) {
        console.error('Failed to update listing:', err);
        setSubmitError('Cập nhật thất bại. Vui lòng kiểm tra lại thông tin và thử lại.');
      } finally {
    ...
    return (
      <DashboardLayout sideNavProps={sideNav} user={landlordUser}>
        <div className="flex flex-col items-center justify-center py-20 text-center max-w-md mx-auto">
          <div className="inline-flex items-center gap-3 px-6 py-3 bg-surface-container rounded-full mb-6">
    ...
    return (
      <DashboardLayout sideNavProps={sideNav} user={landlordUser}>
        <div className="flex flex-col items-center justify-center py-20 text-center max-w-md mx-auto">
          ...
          <h2 className="font-headline font-extrabold text-3xl text-on-surface mb-3">Cập nhật thành công!</h2>
          <p className="text-on-surface-variant mb-8">Tin đăng của bạn đã được cập nhật thành công và đang chờ duyệt.</p>
  ...
  return (
    <DashboardLayout sideNavProps={sideNav} user={landlordUser}>
      {/* Header */}
      <h2 className="font-headline font-extrabold text-2xl tracking-tight text-on-background mb-2">{t('landlord.listing.edit.title', 'Cập nhật tin đăng')}</h2>
      <p className="text-xs text-on-surface-variant mb-8 flex items-center gap-1">
        ...
        {t('landlord.listing.edit.subtitle', 'Các trường có dấu * là bắt buộc điền.')}
      </p>
            ...
            <div className="relative">
              ...
              >
                {isSubmitting ? (
                  ...
                  <>
                    {step === STEPS.length - 1 ? 'Cập nhật' : 'Tiếp tục'}
                    <span className="material-symbols-outlined text-lg">{step === STEPS.length - 1 ? 'save' : 'arrow_forward'}</span>
                  </>
                )}
              </button>
